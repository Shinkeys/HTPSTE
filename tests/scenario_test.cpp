...
TEST(ScenarioTest, EveryDeterministicScenarioCoversTheInstrumentUniverse) {
    ...
    for (const auto& scenario : scenarios) {
        ...
        EXPECT_EQ(scenario->priceShocks().size(), instrument_symbols::all.size());
...
TEST(ScenarioTest, AppliesRelativePriceShock) {
    ...
    EXPECT_DOUBLE_EQ(*scenario.stressedPriceFor(instrument_symbols::apple, 100.0), 72.0);
    EXPECT_EQ(scenario.stressedPriceFor("UNKNOWN", 100.0), std::nullopt);
    EXPECT_THROW(
        static_cast<void>(scenario.stressedPriceFor(instrument_symbols::apple, -1.0)),
...
TEST(ScenarioTest, OilCollapseHitsSaudiAramcoMoreThanTheBroadMarket) {
    ...
    EXPECT_LT(
        *scenario.priceShockFor(instrument_symbols::saudi_aramco),
