...
void validateParameterShifts(const ParameterShiftMap& shifts)
{
    for (const auto& [name, shift] : shifts)
    {
        ...
        {
            throw std::invalid_argument(
                "scenario parameter name must not be blank");
        }
        if (!std::isfinite(shift))
        {
            throw std::invalid_argument(
                "scenario parameter shifts must be finite");
        }
    }
}
...
Scenario::Scenario(PriceShockMap price_shocks,
...
std::optional<double>
Scenario::priceShockFor(const std::string_view symbol) const noexcept
{
...
std::optional<double>
Scenario::parameterShiftFor(const std::string_view name) const noexcept
{
...
std::optional<double>
Scenario::stressedPriceFor(const std::string_view symbol,
                           const double current_price) const
{
    if (!std::isfinite(current_price) || current_price < 0.0)
    {
        throw std::invalid_argument(
            "current price must be finite and non-negative");
    }
