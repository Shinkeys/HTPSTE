    ...
    for (const auto& [symbol, price] : prices)
    {
        if (isBlank(symbol))
        {
            throw std::invalid_argument(
                "market price symbol must not be blank");
        }
        if (!std::isfinite(price) || price < 0.0)
        {
            throw std::invalid_argument(
                "market prices must be finite and non-negative");
        }
    }
}
...
void validateParameters(const ModelParameters& modelParameters)
{
    for (const auto& [name, value] : modelParameters.values)
    {
        ...
        {
            throw std::invalid_argument(
                "model parameter name must not be blank");
        }
...
MarketState::MarketState(PriceMap prices, ModelParameters modelParameters)
    : m_prices(std::move(prices)),
      m_model_parameters(std::move(modelParameters))
{
...
std::optional<double>
MarketState::priceFor(const std::string_view symbol) const noexcept
{
...
std::optional<double>
MarketState::parameter(const std::string_view name) const noexcept
{
